...
function uniqueID(id){
  var ret = id+'',
    count = 0;
  while(document.getElementById(ret)){
    ret = id + count;
    count ++
  }
  return ret
...
for(var i=0; i<inputElements.length; i++){
  if(inputElements[i].type == "password"){
    ...
    var checkID = uniqueID(elm.id || i);
	var checkBox = document.createElement("input");
	checkBox.type = "checkbox";
	checkBox.checked=true;
	checkBox.title="This checkbox deactivates/activates the PaswordHashing!";
	checkBox.id = checkID;
	if(elm.nextSibling){
	  elm.parentNode.insertBefore(checkBox, elm.nextSibling);
    } else {
	  elm.parentNode.appendChild(checkBox);
	}
    elm.form.addEventListener("submit", function(){
      console.log("DA KEY: ",utf8StringCodec.fromBits(sha256.hash(elm.value+""+window.location.host+self.options.salt), true))
